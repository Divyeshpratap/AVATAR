...
class POCTrackGenerator:
    def __init__(self, args):
        ...
        self.video_fps = self.cap.get(cv2.CAP_PROP_FPS) or 25  # Your actual FPS is 30
    ...
    def run(self):
        ...
        while True:
            ...
            # Initialize an empty list to store audio data for the current batch
            audio_buffer = []

            # Read a batch of frames and audio data
            for _ in range(self.batch_size):
                ...
                # Save the original frame to the all_frames directory
                frame_filename = os.path.join(self.all_frames_dir, f'frame_{frame_count:06d}.jpg')
                cv2.imwrite(frame_filename, frame)
                logger.debug(f"Saved original frame {frame_count} to {frame_filename}.")

                # Read the exact number of samples per frame
                audio_chunk = self.stream.read(self.samples_per_frame, exception_on_overflow=False)
