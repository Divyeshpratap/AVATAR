...
class POCTrackGenerator:
    ...
    def detect_faces_s3fd(self, frame):
        ...
        if bboxes is not None:
            for bbox in bboxes:
                x_min, y_min, x_max, y_max = bbox[:-1]  # Extract bounding box coordinates
                prob = bbox[-1]  # Extract confidence score
                detections.append([x_min, y_min, x_max, y_max, prob])  # Format to match SORT output
    ...
    def run(self):
        ...
        """
        logger.info('Starting Optimized Batch Processing with Single TalkNet Initialization...')
        frame_count = 0
            ...
            for idx, frame in enumerate(frames):
                ...
                for trk in tracked_objects:
                    ...
                    if len(self.current_tracks[track_id]['frames']) >= self.batch_size:
                        logger.info(f'Running TalkNet Evaluation for Track {track_id}...')
                        ...
                        # Correct calculation for relative_indices
                        # Calculate the start frame number of the current batch
                        ...
                        if annotated_batch:
                            ...
                            logger.debug(f"Enqueued a batch of {len(annotated_batch)} frames for Track {track_id}.")
